...
$startTime = Get-Date
$routerProviders = @("docker", "file")
do {
    Start-Sleep -Milliseconds 100
    foreach ($provider in $routerProviders) {
        try {
            $status = Invoke-RestMethod "http://localhost:8079/api/http/routers/cm-secure@$provider"
...
} while ($status.status -ne "enabled" -and $startTime.AddSeconds(120) -gt (Get-Date))
